...
pub const JoinProducer = struct {
    ...
    left_row_map: std.StringHashMap(std.ArrayList(u32)),
    ...
    pub fn init(
        ...
        return JoinProducer{
            ...
            .left_row_map = std.StringHashMap(std.ArrayList(u32)).init(allocator),
        };
    }

    pub fn deinit(self: *JoinProducer) void {
    ...
    pub fn next(self: *JoinProducer) !TaskResult {
        if (self.finished) {
        ...
        if (self.left_columns == null) {
            ...
            while (true) {
                ...
                const key_column = (try self.left_projection(self.allocator, chunk, chunk.rows())).Str.slices;
        ...
        const key_column = (try self.right_projection(self.allocator, chunk, chunk.rows())).Str.slices;
