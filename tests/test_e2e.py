...
from mini_spark.execution import ExecutionEngine, ThreadEngine
...
TEST_QUERIES = [
    ...
    (
        "SELECT product, SUM(quantity) AS total_quantity, MAX(price) AS max_price FROM '{orders}' GROUP BY product;",
        to_rows(
...
@pytest.mark.parametrize(("query", "expected_rows"), TEST_QUERIES)
